...
function max_longest_level(node, val) {
  return (node && node.value === val) ? Math.max(max_longest_level(node.left, val), max_longest_level(node.right, val)) +1 : 0
}
...
// 另一种写法
function solve(node) {
  return Math.max(...[...transverse(node)].map(node=>{
    return max_longest_level(o.left, o.value) + max_longest_level(o.right, o.value)
  }))
}